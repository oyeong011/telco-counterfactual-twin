...
The semantic-key policy evaluates parsed snake/kebab/camel tokens and a whole-key lowercase alphanumeric normalization. Exact normalized safe keys are checked first; all other keys use direct sensitive lexemes, URL/URI semantic edges, and order-independent authority/identity/secret groups. Removing separators or reversing group terms therefore cannot turn `identifier_customer`, `payload_push`, or `callback_url` into an accepted key, while `security_level`, `duration_ms`, `shellfish_count`, `tokenization_mode`, and `executioner_state` remain safe.
