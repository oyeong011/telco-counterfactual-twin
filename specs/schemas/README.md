...
The semantic-key policy evaluates parsed snake/kebab/camel tokens and a whole-key lowercase alphanumeric normalization. Exact normalized safe keys are checked first; all other keys use direct sensitive stems (including URL/URI at any position) and order-independent authority/identity/secret groups. API credential groups scan the collapsed key at any position after removing explicitly benign embedded lexemes such as `rapid`, so `rapid_key` remains safe while `serviceapiuserkeyhash` rejects. Removing separators, moving sensitive terms into the middle, or reversing group terms cannot widen the accepted surface; registered natural-word exceptions remain safe only as exact whole normalized values.
