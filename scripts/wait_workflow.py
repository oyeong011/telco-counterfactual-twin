...
def _load_runs(workflow: str, runs_file: Path | None) -> tuple[WorkflowRun, ...]:
...
def _load_logs(run_id: int, logs_file: Path | None) -> str:
...
def main(
    ...
    while True:
        runs = _load_runs(workflow, runs_file)
        ...
        if matching:
            ...
            if run.status == "completed":
                logs = _load_logs(run.database_id, logs_file)
                if _has_auth_blocked_marker(logs):
