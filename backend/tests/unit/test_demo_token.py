...
from .test_memory_store import MutableClock
...
def test_restart_epoch_invalid_and_expired_tokens_have_distinct_domain_semantics() -> None:
    async def scenario() -> None:
        ...
        created = await first.create_session(
            SessionCreate(session_id="session-0001", nonce=b"\x01" * 16)
        )
        ...
        lost = await restarted.access(SessionAccess(token=created.token))
        forged = await first.access(
            SessionAccess(token=created.token[:-1] + ("A" if created.token[-1] != "A" else "B"))
        )
        first_clock.advance_to(NOW + timedelta(minutes=15, seconds=1))
        expired = await first.access(SessionAccess(token=created.token))
...
def test_malformed_and_future_tokens_are_401_semantics() -> None:
    async def scenario() -> None:
        ...
        created = await store.create_session(
            SessionCreate(session_id="session-0001", nonce=b"\x03" * 16)
        )
        assert not isinstance(created, SessionCreateDenied)
        # When: malformed bytes and a pre-issuance assessment instant are used.
        malformed_value = chr(33)
        malformed = await store.access(SessionAccess(token=malformed_value))
        clock.advance_to(NOW - timedelta(seconds=1))
        future = await store.access(SessionAccess(token=created.token))
...
def test_store_never_persists_token_or_displays_hmac_key() -> None:
    async def scenario() -> None:
        ...
        created = await store.create_session(
            SessionCreate(session_id="session-0001", nonce=b"\x04" * 16)
        )
        assert not isinstance(created, SessionCreateDenied)
